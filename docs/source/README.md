...
11. In the second **Test** cell, paste the model version ID (from the deployed model’s Baseten page) in the specified place. Perform any remaining post-processing here and print/save the output for debugging. Run this cell and ensure the deployed model is outputting expected results. In the model's page in Baseten, navigate to the "Logs" tab of the "Draft" version to see the Build and Deployment Logs. These logs can be very helpful for debugging, usually pointing out missing dependencies and providing guidance to what line in `model_name_truss`'s python files failed.
